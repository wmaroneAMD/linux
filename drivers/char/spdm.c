...
static int spdm_open(struct inode *inode, struct file *file)
{
	...
	/* Pin the device struct for the lifetime of this open file. */
	get_device(&sdev->dev);

	down_read(&sdev->ops_sem);
	if (!sdev->ops) {
		rc = -ENODEV;
		goto err_dev;
	}
	...
	/* Enforce the session cap and assign a collision-free, non-zero id. */
	mutex_lock(&sdev->sess_lock);
	if (sdev->active_count >= sdev->ops->max_session_count) {
		mutex_unlock(&sdev->sess_lock);
		rc = -EBUSY;
		goto err_free;
	}
	do {
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (!sdev->ops)
		rc = -ENODEV;
	else if (!sdev->ops->read)
		rc = -EINVAL;
	else
		rc = sdev->ops->read(sdev, file, sf->session_id, buf, count);
	up_read(&sdev->ops_sem);
			  ...
			  size_t count, loff_t *ppos)
{
	...
	if (!sdev->ops)
		rc = -ENODEV;
	else if (!sdev->ops->write)
		rc = -EINVAL;
	else
		rc = sdev->ops->write(sdev, file, sf->session_id, buf, count);
	up_read(&sdev->ops_sem);
...
static long spdm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (!sdev->ops)
		rc = -ENODEV;
	else if (!sdev->ops->ioctl)
		rc = -ENOTTY;
	else
		rc = sdev->ops->ioctl(sdev, file, cmd, arg);
	up_read(&sdev->ops_sem);
...
static void spdm_dev_release(struct device *dev)
{
	...
	mutex_lock(&spdm_idr_lock);
	idr_remove(&spdm_minor_idr, sdev->minor);
	mutex_unlock(&spdm_idr_lock);
...
void spdm_unregister(struct spdm_device *sdev)
{
	...
	down_write(&sdev->ops_sem);
	sdev->ops = NULL;
	up_write(&sdev->ops_sem);
